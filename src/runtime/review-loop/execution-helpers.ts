...
export type NormalizedAutoFixResult = {
  escalatedFromAuto: Array<Record<string, unknown>>;
...
export function normalizeAutoFixResult(autoResult: Record<string, unknown> = Object()): NormalizedAutoFixResult {
  const escalatedFromAuto = Array.isArray(autoResult.escalatedTasks)
    ? (autoResult.escalatedTasks as Array<Record<string, unknown>>)
    : [];
  const stats = autoResult.stats as Record<string, unknown> | undefined;
  const autoFixedCount = Number(stats?.fixed || 0);
  return {
    escalatedFromAuto,
    autoFixedCount,
...
export function autoFixErrorFallback(autoFixTasks: Array<Record<string, unknown>> = []): {
  escalatedFromAuto: Array<Record<string, unknown>>;
  autoFixedCount: number;
} {
  return {
