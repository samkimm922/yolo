...
function readEvidenceDir(taskRoot) {
  const findings = [];
...
function extractFragilityPoints(task = Object(), outcome = Object()) {
  ...
  if (outcome.status === "failed" || outcome.status === "blocked") {
    points.push(`任务以 ${outcome.status} 结束: ${(outcome.reason || "").slice(0, 120)}`);
  }

  // Fragility from cross-file changes
  const files = (task.scope?.targets || []).map((target) => clean(target.file || target));
  if (files.length > 2) {
...
function extractAssumptionChanges(task = Object(), outcome = Object(), evidenceFindings = []) {
  ...
  for (const finding of evidenceFindings) {
    if (finding.type === "assumption_change" || finding.code === "ASSUMPTION_CHANGED") {
      changes.push(finding.message || finding.summary || JSON.stringify(finding).slice(0, 200));
    }
  ...
  const targets = (task.scope?.targets || []).map((target) => clean(target.file || target));
  ...
  if (outcome.status === "completed") {
    const postConditions = task.post_conditions || [];
    const failConditions = postConditions.filter((cond) => (cond.severity || "FAIL") === "FAIL");
...
export function buildTaskSummary({ task = Object(), outcome = Object(), evidenceDir = "", projectRoot = "" } = Object()) {
  ...
  const summary = status === "completed"
    ...
    : `${status}: ${title} — ${(outcome.reason || "").slice(0, 100)}`;
