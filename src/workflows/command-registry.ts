...
export const YOLO_COMMANDS = [
  stableCommand({
    ...
    workflow: "demand",
    description: "Clarify an idea through an automatic six-step continuation, with explicit stage overrides for advanced use.",
    argumentHint: "<idea> [status|dispatch|--stage brainstorm|discuss|office-hours|plan|discover|prd]",
    objective: "Act as the YOLO demand facilitator. Ask exactly one next_question when required slots are missing, keep assumptions separate from verified facts, and continue from the latest verified .yolo artifact until the approved PRD is generated.",
    mode: "demand",
    aliases: ["yolo demand", "yolo-demand"],
    writes_code: false,
    requires_confirmation: false,
    safety: "Demand and PRD only. 不改代码, do not edit business code, and do not advance past a blocked step. 缺槽位时只返回一个 next_question, 不输出大段建议. 批准最后: demand approval is not code-execution authorization.",
    usage: "yolo demand \"我想把库存预警需求聊清楚\"",
  }),
  stableCommand({
    name: "auto",
    lifecycle_stage: "idea",
  ...
  internalCommand({
    ...
    lifecycle_stage: "prd",
    workflow: "prd",
    description: "Compile approved demand artifacts into a draft PRD/spec; executable only after preflight.",
    argumentHint: "[--discovery <discovery.json>|--demand <session.json|dir>] [--output <prd.json>] [--json]",
    objective: "Generate or validate a draft spec with atomic tasks, preconditions, postconditions, acceptance checks, and traceability; execution still requires preflight.",
    mode: "prd",
