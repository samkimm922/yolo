...
export function buildDefaultReleaseCandidateReports(input: Record<string, unknown> = {}) {
  ...
  return {
    ...
    // review, acceptance, delivery: these reports are not auto-generated by the
    // default runner because they require lifecycle stage evidence (run, review,
    // acceptance) that only exists after a full yolo run pipeline. Generate
    // placeholder blockers that tell the operator exactly what's needed and how
    // to provide it, instead of silently omitting them (which caused the decision
    // gate to fail with a generic RC_GATE_REPORT_BLOCKED).
    review: releaseCandidateReport({
      source: "review",
      blockerCode: "RC_GATE_REVIEW_REPORT_NOT_PROVIDED",
      blockerMessage: "Review report is required but not auto-generated. Run `yolo review` to produce review evidence, or provide it via the `reports.review` SDK option.",
    }),
    acceptance: releaseCandidateReport({
      source: "acceptance",
      blockerCode: "RC_GATE_ACCEPTANCE_REPORT_NOT_PROVIDED",
      blockerMessage: "Acceptance report is required but not auto-generated. Run `yolo accept` to produce acceptance evidence, or provide it via the `reports.acceptance` SDK option.",
    }),
    delivery: releaseCandidateReport({
      source: "delivery",
      blockerCode: "RC_GATE_DELIVERY_REPORT_NOT_PROVIDED",
      blockerMessage: "Delivery report is required but not auto-generated. Complete the delivery lifecycle stage, or provide it via the `reports.delivery` SDK option.",
    }),
