...
- `tools/install-agent-bridge.mjs` 已提供 project/user scope 安装计划和写入器：project scope 写 `AGENTS.md`、`CLAUDE.md`、`.codex/skills/yolo`、`.codex/skills/yolo-*`、`.claude/skills/yolo`、`.claude/commands/yolo*.md`；user scope 写 `~/.agents/skills/yolo`、`~/.agents/skills/yolo-*`、`~/.agents/skills/source-command-yolo*`、`~/.claude/skills/yolo`、`~/.claude/commands/yolo*.md`；dry-run 不写盘，真实写入会保留已有文件并用 `--force` 才覆盖非 bridge 产物。
...
- `src/**/*.ts` 168 个，`__tests__/*.test.ts` 121 个；`docs/yolo-public-sdk-progress.md` 与本文的结构数字已由 `docs-truth-sync` 测试锁住。
