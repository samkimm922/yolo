...
- 文件归位：CLI、core、prd、spec、runtime、agents、review、evidence、adapters 分层；root `.ts` public/compat entry 已收敛到 8 个，非 public root 脚本已批量下沉到 `src/`/`bin/`。
- runner 解耦：`runner.ts` 已从旧单体降到 13 行兼容入口；review/retry/run lifecycle pipeline、task execution loop、main loop、split PRD 写回、runner helper、embedded progress server、shutdown/timeout/fatal cleanup、context、progress log、recovery checkpoint、task runtime bindings、process handlers 和 runtime API freeze inspector 都已进入 `src/runtime/*` 可测模块；internal `src/runtime/runner-core.ts` 已压到 596 行。
...
- root script migration：`atomic-task-doctor.mjs`、`prd-contract-doctor.mjs`、`provider-doctor.mjs`、`pm`、`audit-to-prd`、`contract`、`scanner`、`validate-prd`、progress/devtool/gate helper、`learn`、`task-logger`、`state-snapshot`、`session-memory` 类实现已迁入 `src/*` 或 `bin/*`；根目录当前只保留 8 个 compatibility/public entry，且 `migrate_to_src` 清零。
...
- 跨项目 fixtures：已有 experimental fixture registry、隔离执行 harness，并覆盖 `node-basic`、`no-tests`、`python-basic`、`python-service`、`frontend-vite`、`backend-api`、`monorepo`、`dirty-tree`、`failing-baseline`；真实大型项目仍待补。
...
- `package.json` 已有 49 个 package exports、6 个 bin 和显式 `files` allowlist：`yolo`、`yolo-pi`、`yolo-gate`、`yolo-prompt`、`yolo-prd-preflight`、`yolo-prd-migrate-gates`；`./pm`、`./audit-to-prd`、`./contract`、`./scanner`、`./validate-prd`、`./runtime/adapter-evidence`、`./runtime/progress-ui-evidence`、`./eval/benchmark`、`./release/hardening-drill`、`./release/decision-gate`、`./release/operator-state`、`./release/operator-runbook`、`./release/post-release-audit`、`./release/stable-graduation`、`./release/manual-external-release`、`./release/agent-integration-doctor`、`./release/real-project-dogfood`、`./release/pi-execution-drill`、`./release/runtime-boundary-decision`、`./release/public-beta-evidence`、`./release/real-project-dogfood-pack`、`./release/experience-pack-audit` 和 `./release/nontechnical-ux-doctor` 已指向 `src/`。
- `docs/sdk-agent-architecture.md` 已明确 PI 是最高阶 preset，SDK 不应 PI-only。
- 根目录当前有 9 个 `.ts` 文件：8 个 public/compat entry（`sdk.ts`、`runner.ts`、`gate.ts`、`prompt.ts`、`learn.ts`、`task-logger.ts`、`state-snapshot.ts`、`session-memory.ts`）和 1 个测试配置；其中只有 `sdk.ts` 是允许保留的 root SDK facade，其余 public/compat entry 都指向 `src`/`bin` 或 `dist` runtime。
...
- `tools/install-agent-bridge.mjs` 已提供 project/user scope 安装计划和写入器：project scope 写 `AGENTS.md`、`CLAUDE.md`、`.codex/skills/yolo`、`.claude/skills/yolo`、`.claude/commands/yolo*.md`；user scope 写 `~/.agents/skills/yolo`、`~/.claude/skills/yolo`、`~/.claude/commands/yolo*.md`；dry-run 不写盘，真实写入会保留已有文件并用 `--force` 才覆盖非 bridge 产物。
...
- `src/**/*.ts` 164 个，`__tests__/*.test.ts` 120 个；`docs/yolo-public-sdk-progress.md` 与本文的结构数字已由 `docs-truth-sync` 测试锁住。
...
- fixture harness 已抽到 `src/fixtures/harness.mjs`，可以把 fixture 复制到临时目录、运行 smoke command 并写入 evidence artifact。
...
| runner task loop | 有执行、retry、review、gate；review/retry/run pipeline、task execution loop、main loop、split PRD 写回、runner helper、embedded progress server、shutdown lifecycle、context、checkpoint、process handlers 和 task runtime bindings 已拆到 internal modules，根入口是 13 行 shim，runner-core 600 行。 | Ralph 单 story fresh loop、GSD phase execution。 | billable execution 和真实 dogfood 仍缺证据。 | operator evidence |
...
目标：继续把已从 2778 行降到 13 行根入口的 runner 拆成可测组件，并把后续风险转向 release hardening。
...
- P27-01 native agent integration：`tools/install-agent-bridge.mjs` 从项目 memory/skill 安装升级为 project/user scope 安装器；Claude Code 获得真实 `.claude/commands/yolo*.md` slash commands，Codex 获得 `~/.agents/skills/yolo` native skill 与 command aliases，package tarball 包含安装器和非技术用户文档；全量 627 tests / 107 suites / 0 fail。
...
- P39-01 non-technical UX doctor：新增 `src/release/nontechnical-ux-doctor.mjs`，校验 README、agent docs、native skill、Claude/Codex command artifacts 都收束到 `/yolo 你的需求，先只生成计划，不要改代码。` 和 chat-first 规则。
