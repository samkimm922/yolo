...
需求沟通统一使用 `/yolo-demand`：brainstorm、discover、discuss 通过 `--stage` 选择，office-hours 通过 `--mode office-hours` 选择。计划和 PRD/spec 分别使用 `/yolo-tasks` 与 `/yolo-spec`；旧顶层命令不再安装或路由。
