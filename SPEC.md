...
- **UI 完成定义必须 declare-first。** 优先复用项目已有的声明式验收 adapter；没有时，在需求阶段用现有单点追问收集用户声明的入口/命令、可观察结果和证据，并在 PRD 生成时写入 `.yolo/adapters/`。两种声明都没有时继续 fail-closed，但必须返回一个具体、可回答的追问，禁止系统猜选 adapter。
